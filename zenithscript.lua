-- [[ ZYROX VN - V40.0 (THE FINAL REDEMPTION) ]] --
-- Đầy đủ 7 Danh mục. Lõi Farm Redz (Bypass TP, Hitbox 60, Fast Attack).
-- Tối ưu hóa 100% chống đen màn hình trên giả lập Mobile/LDPlayer.
...
_G.SelectWeapon = "Melee"; _G.AutoStats = false; _G.StatsPoint = 1
_G.Stats = {Melee = false, Defense = false, Sword = false, BloxFruit = false}
_G.ESPPlayer = false; _G.ESPFruit = false; _G.ESPChest = false
local StartBring = false; local PosMon = nil; local MonFarm = ""

-- =========================================================
-- KHỞI TẠO GIAO DIỆN (CHỐNG LỖI ĐEN MÀN HÌNH 100%)
-- =========================================================
local UI_NAME = "ZyroxTrueHub_V40"
...
Title.Text = "ZYROX VN <font color='#00d2ff'>• V40 (FINAL REDEMPTION)</font>"
...
local function createButton(page, labelText, callback)
    local btn = Instance.new("TextButton", page); btn.Size = UDim2.new(0.94, 0, 0, 30); btn.BackgroundColor3 = Color3.fromRGB(19, 25, 36); btn.TextColor3 = Color3.fromRGB(0, 210, 255); btn.Font = Enum.Font.GothamMedium; btn.TextSize = 11; btn.Text = labelText
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)
    btn.MouseButton1Click:Connect(function() if callback then callback() end end)
end

-- TRẢ LẠI ĐỦ 7 DANH MỤC
local pFarm = createTab("Farm", "🌾", "Cày Cấp (Farm)")
local pStats = createTab("Stats", "📈", "Nâng Điểm")
local pFruit = createTab("Fruits", "🍎", "Trái Ác Quỷ")
...
-- [ TAB FARM ]
local wSeg = Instance.new("Frame", pFarm); wSeg.Size = UDim2.new(0.94, 0, 0, 28); wSeg.BackgroundColor3 = Color3.fromRGB(15, 18, 25); Instance.new("UICorner", wSeg).CornerRadius = UDim.new(0, 6)
local wLay = Instance.new("UIListLayout", wSeg); wLay.FillDirection = Enum.FillDirection.Horizontal; wLay.HorizontalAlignment = Enum.HorizontalAlignment.Center; wLay.VerticalAlignment = Enum.VerticalAlignment.Center; wLay.Padding = UDim.new(0, 3)
local wList = {{name = "Melee", label = "🥊 Melee"}, {name = "Sword", label = "⚔️ Sword"}, {name = "Blox Fruit", label = "🍎 Fruit"}}
for _, d in ipairs(wList) do
    local b = Instance.new("TextButton", wSeg); b.Size = UDim2.new(0.3, 0, 0.78, 0); b.BackgroundColor3 = _G.SelectWeapon == d.name and Color3.fromRGB(0, 160, 240) or Color3.fromRGB(28, 35, 48); b.TextColor3 = _G.SelectWeapon == d.name and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(140, 150, 170); b.Font = Enum.Font.GothamMedium; b.TextSize = 10; b.Text = d.label; Instance.new("UICorner", b).CornerRadius = UDim.new(0, 4)
    b.MouseButton1Click:Connect(function()
        _G.SelectWeapon = d.name
        for _, btn in pairs(wSeg:GetChildren()) do if btn:IsA("TextButton") then btn.BackgroundColor3 = btn.Text == d.label and Color3.fromRGB(0, 160, 240) or Color3.fromRGB(28, 35, 48); btn.TextColor3 = btn.Text == d.label and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(140, 150, 170) end end
    end)
end
...
createToggle(pFarm, "🧲 Gom Quái & Khóa Vật Lý", true, function(v) _G.BringMob = v end)
createToggle(pFarm, "⚔️ Đánh Siêu Tốc (Fast Attack)", true, function(v) _G.FastAttack = v end)
...
createButton(pFruit, "📦 Store All Fruits", function()
    for _, v in pairs(Plr.Backpack:GetChildren()) do if v:IsA("Tool") and string.find(v.Name, "Fruit") then CommF:InvokeServer("StoreFruit", string.split(v.Name, "-")[1], v) end end
end)
            ...
            pcall(function()
                ...
                elseif not string.find(pGui.Main.Quest.Container.QuestTitle.Title.Text, mN) then
                    ...
                    if tMob then
                        ...
                        -- ĐỨNG CÁCH QUÁI 8 MÉT (100% CHÉM TRÚNG)
                        ...
                        if dist <= 10 then
                            MonFarm = mN; StartBring = true
                            if _G.BringMob then
                                for _, m in pairs(Workspace.Enemies:GetChildren()) do
                                    if m.Name == mN and m:FindFirstChild("Humanoid") and m.Humanoid.Health > 0 then
                                        m.HumanoidRootPart.CFrame = PosMon
    ...
    while task.wait(0.01) do
        if _G.AutoFarm and _G.FastAttack then
            pcall(function()
                ...
                if not ac then
                    local g_up = debug.getupvalues or getupvalues
                    if g_up then for _, v in pairs(g_up(CbFw)) do if type(v) == "table" and v.activeController then ac = v.activeController break end end end
                end
...
-- =========================================================
-- VÒNG LẶP ESP (Player, Chest, Fruit)
-- =========================================================
task.spawn(function()
