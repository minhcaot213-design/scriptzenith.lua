...
-- ===================================================
-- 4. HỆ THỐNG ESP (FIX GIẬT - DÙNG RENDERSTEP + POOLING)
-- ===================================================
local espCache = {} -- Lưu BillboardGui để tái sử dụng, tránh tạo/destroy liên tục
...
local function hideESP(parent, name)
    if parent and parent:FindFirstChild(name) then
        parent[name].Enabled = false
    end
end

-- ESP Player (cập nhật mỗi 3 frame để đỡ giật)
local frameCounter = 0
RunService.RenderStepped:Connect(function(delta)
    frameCounter = frameCounter + 1
    if frameCounter % 3 ~= 0 then return end -- chỉ update 1/3 số frame
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            ...
            if espPlayerEnabled and head and hum and hum.Health > 0 then
                ...
                local bb = getOrCreateBillboard(head, "Zenith_PlayerBillboard", 
...
-- Fruit ESP + Chest ESP (gộp chung, update mỗi 0.3s để tránh giật)
task.spawn(function()
    while true do
        task.wait(0.3)
        local myHRP = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        ...
        -- Chest ESP (dùng cache để tránh quét lại quá nhiều)
...
-- ===================================================
-- 5. HỆ THỐNG FARM + FLY (FIX 100% - KHÔNG BỊ GIẬT, ĐÁNH NHANH)
-- ===================================================
...
-- Hàm lấy nhiệm vụ theo level (placeholder - anh có thể thay bằng logic thật)
...
local function checkHasQuest()
    -- Placeholder - nếu có logic check quest thật thì thay vào
    return true
end
...
local function equipChosenWeapon()
    -- Placeholder - tự trang bị vũ khí theo selectedWeaponType
end
...
task.spawn(function()
    while true do
        ...
        root.AssemblyLinearVelocity = Vector3.new(
            root.AssemblyLinearVelocity.X,
            math.max(root.AssemblyLinearVelocity.Y, -3),
            root.AssemblyLinearVelocity.Z
        )
...
-- ===================================================
-- 6. CÁC TÍNH NĂNG PHỤ (SPEED, JUMP, FRUIT, WATER...)
-- ===================================================
