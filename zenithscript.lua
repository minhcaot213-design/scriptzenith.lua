-- [[ ZENITH BLOX FRUIT - V16.0 (LDPLAYER PERFECT BACKGROUND)
--    FIX KẸT BAY BẰNG CFRAME STEP + SKY FARM 7M DƯỚI CHÂN CHỐNG ĐÁNH LẠI
-- ]] --
...
-- =========================================================
-- ANTI AFK (Trống Disconnect khi treo Background)
-- =========================================================
...
local UI_NAME = "ZenithBloxFruit_Zyrox_V16"
...
local LangDict = {
    VI = {
        title = "ZYROX VN <font color='#00d2ff'>• V16.0 (PRO)</font>",
        ...
        auto_farm_level = "⚡ Tự Động Farm (Mượt CFrame)", auto_quest = "📜 Tự Nhận Nhiệm Vụ", bring_mob = "🧲 Treo Quái Dưới Chân (7m)",
    ...
    EN = {
        ...
        auto_farm_level = "⚡ Auto Farm (Smooth CFrame)", auto_quest = "📜 Auto Quest", bring_mob = "🧲 Sky Bring Mobs (7m)",
...
-- =========================================================
-- MAIN GUI (GIỮ NGUYÊN KIẾN TRÚC UI HOÀN HẢO)
-- =========================================================
...
Banner.Name = "Banner"; Banner.Parent = Sidebar; Banner.Size = UDim2.new(1, -14, 0, 82); Banner.Position = UDim2.new(0, 7, 0, 7); Banner.BackgroundColor3 = Color3.fromRGB(18, 23, 32); Banner.BackgroundTransparency = 0; Banner.BorderSizePixel = 0; Banner.ScaleType = Enum.ScaleType.Crop
local BannerStroke = Instance.new("UIStroke", Banner); BannerStroke.Color = Color3.fromRGB(0, 190, 255); BannerStroke.Thickness = 1; Instance.new("UICorner", Banner).CornerRadius = UDim.new(0, 4)

local TabScroller = Instance.new("ScrollingFrame", Sidebar)
TabScroller.Name = "TabScroller"; TabScroller.Size = UDim2.new(1, -8, 1, -98); TabScroller.Position = UDim2.new(0, 4, 0, 94); TabScroller.BackgroundTransparency = 1; TabScroller.BorderSizePixel = 0; TabScroller.ScrollBarThickness = 3; TabScroller.ScrollBarImageColor3 = Color3.fromRGB(0, 190, 255); TabScroller.ScrollBarImageTransparency = 0.15; TabScroller.CanvasSize = UDim2.new(0, 0, 0, 0); TabScroller.AutomaticCanvasSize = Enum.AutomaticSize.Y; TabScroller.ScrollingDirection = Enum.ScrollingDirection.Y
...
-- =========================================================
-- SUPER FAST ATTACK (COMBAT FRAMEWORK NATIVE HACK)
-- =========================================================
...
-- Xóa Animation tay của bạn để tránh lag giả lập
...
-- =========================================================
-- HỆ THỐNG BAY C-FRAME (CHỐNG KẸT TƯỜNG 100%)
-- =========================================================
local function toTargetPos(targetCFrame)
    ...
    -- Bay từng bước 15 studs (tương đương 300 tốc độ/s) không bị kẹt tường
    root.CFrame = CFrame.lookAt(root.Position, targetCFrame.Position) * CFrame.new(0, 0, -math.min(dist, 15))
end
...
-- =========================================================
-- LOGIC TÌM NHIỆM VỤ & QUÁI (LỌC BOSS CHUẨN XÁC)
-- =========================================================
...
-- TÌM ĐÚNG TÊN QUÁI VẬT, LOẠI BỎ BOSS (Ví dụ: Gorilla != The Gorilla King)
...
-- =========================================================
-- VÒNG LẶP FARM CHÍNH: BẠN Y+30, QUÁI Y+23 (TẦM ĐÁNH HOÀN HẢO)
-- =========================================================
...
task.spawn(function()
    while task.wait(0.05) do
        if AutoFarmLevel and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and CommF then
            ...
            -- Chống rớt xuống biển khi đang bay
            local myBv = myHRP:FindFirstChild("Zenith_AntiGravity")
            if not myBv then
                myBv = Instance.new("BodyVelocity", myHRP)
                myBv.Name = "Zenith_AntiGravity"
            ...
            if currentQuest then
                ...
                if #mobList > 0 then
                    ...
                    if primaryHRP then
                        ...
                        -- VỊ TRÍ CHIẾN LƯỢC: Bạn ở tít trên trời cao 30 mét
                        local safePlayerPos = CFrame.new(groundPos.X, groundPos.Y + 30, groundPos.Z)
                        
                        -- Quái sẽ được hút lên ở mức Y + 23 (Thấp hơn bạn 7 mét, quái đánh toàn hụt)
                        local mobTargetPos = CFrame.new(groundPos.X, groundPos.Y + 23, groundPos.Z)
                        ...
                        if distance > 10 then
                            ...
                            -- Đứng im nhìn thẳng xuống đầu bãi quái
                            myHRP.CFrame = CFrame.lookAt(safePlayerPos.Position, mobTargetPos.Position)
                            ...
                            if BringMob then
                                for _, otherMob in ipairs(mobList) do
                                    ...
                                    if oHRP and oHum and oHum.Health > 0 and (oHRP.Position - groundPos).Magnitude <= 350 then
                                        pcall(function()
                                            -- Đệm khí chống trọng lực cho quái
                                            local oBv = oHRP:FindFirstChild("Zenith_AntiGravity")
                                            if not oBv then
                                                oBv = Instance.new("BodyVelocity", oHRP)
                                                oBv.Name = "Zenith_AntiGravity"
                                                oBv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                                            end
                                            oBv.Velocity = Vector3.zero
                                            
                                            -- Gắn chặt quái ở tọa độ dưới chân bạn
                                            oHRP.CFrame = mobTargetPos
                                            oHRP.Size = Vector3.new(4, 5, 2) -- Hitbox nhỏ chuẩn của game, chống Anti-cheat
                    ...
                    if spawnCFrame then
                        -- Bay lên trời ngay từ đầu để không vướng nhà/núi
            ...
            if hrp and hrp:FindFirstChild("Zenith_AntiGravity") then hrp.Zenith_AntiGravity:Destroy() end
        end
    end
end)
