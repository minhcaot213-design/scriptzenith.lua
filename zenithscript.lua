-- [[ ZYROX VN - V43.0 (THE IMMORTAL CORE) ]] --
-- Fix triệt để lỗi đứt gãy vòng lặp Farm (Chờ lệnh...).
-- Tối ưu hóa: Bay cao 20m, Hút quái xuống dưới chân 10m, Hitbox 60, Fast Attack Chuẩn.
...
-- =========================================================
local UI_NAME = "ZyroxFlawless_V43"
local targetParent = nil
local s, p = pcall(function() return gethui() end)
if s and p then targetParent = p else targetParent = game:GetService("CoreGui") end

pcall(function() if targetParent:FindFirstChild(UI_NAME) then targetParent[UI_NAME]:Destroy() end end)
pcall(function() if LocalPlayer:FindFirstChild("PlayerGui") and LocalPlayer.PlayerGui:FindFirstChild(UI_NAME) then LocalPlayer.PlayerGui[UI_NAME]:Destroy() end end)
...
Title.Text = "ZYROX VN <font color='#00d2ff'>• V43.0 (IMMORTAL CORE)</font>"
...
-- =========================================================
-- LOGIC DI CHUYỂN, BTP VÀ TWEEN MƯỢT CỰC AN TOÀN
-- =========================================================
local function BTP(targetCFrame)
    pcall(function()
        if (targetCFrame.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude >= 1500 then
...
local function GetQuestData()
    ...
    if World1 then
        ...
        elseif lvl <= 174 then return "SkyQuest", 1, "Sky Bandit", CFrame.new(-4842, 718, -2621)
        elseif lvl <= 189 then return "SkyQuest", 2, "Dark Master", CFrame.new(-4842, 718, -2621)
        elseif lvl <= 209 then return "PrisonerQuest", 1, "Prisoner", CFrame.new(5307, 2, 474)
...
-- =========================================================
-- VÒNG LẶP AUTO FARM CHÍNH (FAIL-SAFE 100%)
-- =========================================================
task.spawn(function()
        ...
        if _G.AutoFarm then
            local success, err = pcall(function()
                ...
                if not questGui or not questGui.Visible then
                    if (LocalPlayer.Character.HumanoidRootPart.Position - questPos.Position).Magnitude > 15 then
                        topos(questPos)
                    else
                    ...
                    if not string.find(questTitle, mobName) then
                        ...
                        if targetMob then
                            local tHRP = targetMob.HumanoidRootPart
                            -- BAY LÊN ĐẦU QUÁI CAO 20 MÉT CHO AN TOÀN
                            topos(tHRP.CFrame * CFrame.new(0, 20, 0))
                            
                            -- HÚT QUÁI XUỐNG DƯỚI CHÂN CÁCH 10 MÉT
                            if _G.BringMob and shortest < 300 then
                                for _, m in pairs(Workspace.Enemies:GetChildren()) do
                                    if m.Name == mobName and m:FindFirstChild("Humanoid") and m:FindFirstChild("HumanoidRootPart") and m.Humanoid.Health > 0 then
                                        local eHrp = m.HumanoidRootPart
                                        -- HITBOX 60 + Ép xuống dưới chân
                                        eHrp.Size = Vector3.new(60, 60, 60)
                                        eHrp.CFrame = CFrame.new(LocalPlayer.Character.HumanoidRootPart.Position.X, LocalPlayer.Character.HumanoidRootPart.Position.Y - 10, LocalPlayer.Character.HumanoidRootPart.Position.Z)
                            ...
                            -- HẾT QUÁI BAY VỀ CHỖ NHẬN QUEST
                            topos(questPos)
                        end
                    end
                end
            end)
            if not success then warn("Zenith Farm Loop Error:", err) end
        else
